...
SKILL_SPEC_JSON = (
    '{"schema":"scientific-lsp-skill/v1","name":"abinit","software":"abinit","dis'
    'play_name":"ABINIT","description":"ABINIT input preflight for .abi and .abin'
    'it input files.","package":{"name":"abinit-lsp","install":"pip install abini'
    't-lsp"},"entrypoints":{"server":"abinit-lsp","tool":"abinit-lsp-tool"},"file'
    '_patterns":["*.abi","*.abinit"],"operations":["capabilities","check","contex'
    't","complete","hover","symbols","fix"],"diagnostic_contract":"DiagnosticEnve'
    'lope/v1","blocking_policy":{"mode":"fail-on-blocking","description":"Use --f'
    'ail-on-blocking when generated inputs must be launch-ready."},"source_proven'
    'ance":[{"kind":"official_docs","label":"ABINIT Input Variables","url":"https'
    '://docs.abinit.org/variables/"}]}'
)
...
SKILL_MD_LINES = [
    ...
    '',
    (
        'Use this skill when preparing, repairing, or reviewing ABINIT input files be'
    ...
    '',
    (
        'This installs the `abinit-lsp` language server and the `abinit-lsp-tool` age'
        'nt CLI from the `abinit-lsp` Python package.'
    ),
    '',
    ...
    (
        'abinit-lsp-tool context <input-file-or-dir> --line 0 --character 0 --format '
        'json'
    ),
    (
        'abinit-lsp-tool hover <input-file-or-dir> --line 0 --character 0 --format js'
        'on'
    ),
    (
        'abinit-lsp-tool complete <input-file-or-dir> --line 0 --character 0 --format'
        ' json'
    ),
